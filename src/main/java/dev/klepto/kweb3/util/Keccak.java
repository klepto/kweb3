...
//        state[rateBits >>> 6] ^= in & (-1L >>> -inBits);
    ...
    public static String hash(String input) {
        return hash(input, StandardCharsets.US_ASCII);
    }

    public static String hash(String input, Charset charset) {
        return String.format("%064x", new BigInteger(1, rawHash(input, charset)));
    }

    public static byte[] rawHash(String input) {
        return rawHash(input, StandardCharsets.US_ASCII);
    }

    public static byte[] rawHash(String input, Charset charset) {
